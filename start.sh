...
function help() {
	echo "Usage: $(basename "$0") [-a <AUTH_TOKEN>] [-c|--config <CONFIG_ENVFILE>] [-n|--name <CONTAINER_NAME>] [--use-host-net] [-r|--add-route <NETWORK>]*

where:
-a|--auth-token <AUTH_TOKEN>  authentication token/one time password
...
CONTAINERNAME=connect-tunnel-vpn
OTP=
USEHOSTNET=n
ROUTES=()
...
while true ; do
	...
	if docker logs "$CONTAINERID" | grep "VPN connection is stable" >/dev/null ; then
...
if [[ "$USEHOSTNET" != 'y' ]] ; then
	...
	# enable forwarding
	docker exec -it "$CONTAINERNAME" iptables -t nat -A POSTROUTING -o tun0 -j MASQUERADE
